...
const Fights = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Refresh fights
      const refreshed = await fetch("http://localhost:3000/api/fights").then((r) => r.json());
  ...
  return (
      <Box maxW="7xl" mx="auto" p={6}>
        <Heading textAlign="center" mb={8}>Dragon Fights</Heading>
          ...
          {fights.map((fight) => (
              ...
              >
                ...
                <Flex direction={{ base: "column", md: "row" }} align="center" justify="space-between">
                  <VStack spacing={2}>
                    <Image src={fight.challenger.dragon.image} alt={fight.challenger.dragon.name} boxSize="150px" />
                    <Text fontWeight="bold">{fight.challenger.dragon.name}</Text>
                    <Text fontSize="sm" color="gray.500">Rider: {fight.challenger.rider.username}</Text>
                  </VStack>
                  ...
                  <VStack mx={4} spacing={1} textAlign="center">
                    ...
                    {fight.status === "cancelled" ? (
                        ...
                        <>
                          ...
                          <Button size="sm" colorScheme="blue" mt={2} onClick={() => handleOpenForm(fight)}>
                            Update Result
                          </Button>
                        </>
                    ) : fight.result.isDraw ? (
                        ...
                        <>
                          <Text color="green.400">Winner: {fight.result.winner?.name}</Text>
                          <Text color="red.400">Loser: {fight.result.loser?.name}</Text>
                        </>
                    )}
                    {fight.status !== "cancelled" && !fight.result?.isDraw && (
                        <Text fontSize="sm" color="gray.400">
                          Score: {fight.result.winnerScore} - {fight.result.loserScore}
                        </Text>
                    )}
                  </VStack>

                  <VStack spacing={2}>
                    <Image src={fight.opponent.dragon.image} alt={fight.opponent.dragon.name} boxSize="150px" />
                    <Text fontWeight="bold">{fight.opponent.dragon.name}</Text>
                    <Text fontSize="sm" color="gray.500">Rider: {fight.opponent.rider.username}</Text>
                  </VStack>
                </Flex>

                <Divider my={4} />
                <Box fontSize="sm" color="gray.600">
                  <Text><strong>Location:</strong> {fight.fightDetails.location}</Text>
                  <Text><strong>Rounds:</strong> {fight.fightDetails.rounds}</Text>
                  <Text><strong>Notes:</strong> {fight.fightDetails.notes}</Text>
                </Box>
              </Box>
          ))}
        </SimpleGrid>
          ...
          <ModalContent>
            ...
            <ModalBody>
              <FormControl mb={3}>
                ...
                <Input type="number" name="challengerScore" value={formData.challengerScore} onChange={handleChange} />
              </FormControl>
              <FormControl mb={3}>
                <FormLabel>Opponent Score</FormLabel>
                <Input type="number" name="opponentScore" value={formData.opponentScore} onChange={handleChange} />
              </FormControl>
              <FormControl mb={3}>
              ...
              <FormControl display="flex" alignItems="center" mb={3}>
                ...
                <Switch name="isDraw" isChecked={formData.isDraw} onChange={handleChange} />
              </FormControl>
              {!formData.isDraw && (
                  <FormControl mb={3}>
                    <FormLabel>Winner Dragon ID</FormLabel>
                    <Input name="winnerDragonId" value={formData.winnerDragonId} onChange={handleChange} />
                  </FormControl>
              )}
            </ModalBody>
            <ModalFooter>
