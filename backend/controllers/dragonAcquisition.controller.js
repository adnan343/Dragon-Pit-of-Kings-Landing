...
export const acquireDragon = async (req, res) => {
    ...
    // Validate input
    if (!userId || !dragonId) {
        return res.status(400).json({ success: false, msg: "User ID and Dragon ID are required." });
    }

    // Validate IDs format
    if (!mongoose.Types.ObjectId.isValid(userId) || !mongoose.Types.ObjectId.isValid(dragonId)) {
    ...
    try {
        ...
        if (user.userType.toLowerCase() !== "dragon rider") {
            return res.status(403).json({ success: false, msg: "Only Dragon Riders can acquire dragons." });
        }
        ...
        if (dragon.rider) {
            return res.status(409).json({ success: false, msg: "This dragon is already acquired." });
        }
