...
const isUserDragonRider = async (userId, dragonId) => {
    ...
    return dragon && dragon.rider && dragon.rider.toString() === userId.toString();
};

// Initiate a new fight between dragons
export const initiateFight = async (req, res) => {
    try {
        const { challengerDragonId, opponentDragonId, riderId, location, notes } = req.body;

        // Validate required fields
        if (!challengerDragonId || !opponentDragonId || !riderId) {
            return res.status(400).json({
                message: 'Missing required fields: challengerDragonId, opponentDragonId, and riderId are required'
            });
        }
...
export const completeFight = async (req, res) => {
    try {
        const { fightId, winnerDragonId, isDraw, challengerScore, opponentScore, rounds, riderId } = req.body;
        ...
        if (fight.status === 'completed') {
            return res.status(400).json({ message: 'This fight is already completed' });
        }
        ...
        if (riderId) {
            ...
            if (!isUserInvolved) {
                return res.status(403).json({ message: 'Only riders involved in the fight can complete it' });
            }
        }
        ...
        if (isDraw) {
            ...
            if (!isWinnerInFight) {
                return res.status(400).json({ message: 'Winner dragon must be one of the dragons in the fight' });
            }

            // Set winner and loser
            fight.result.winner = winnerDragonId;
            fight.result.loser = fight.challenger.dragon.toString() === winnerDragonId ?
                fight.opponent.dragon : fight.challenger.dragon;
            fight.result.isDraw = false;
        } else {
            return res.status(400).json({ message: 'Either winnerDragonId or isDraw must be provided' });
        }
...
const updateDragonFightRecords = async (fight) => {
    try {
        ...
        if (fight.result.isDraw) {
            // It's a draw
            challengerDragon.fighting.draws = (challengerDragon.fighting.draws || 0) + 1;
            ...
            if (fight.result.winner.toString() === challengerDragon._id.toString()) {
                // Challenger won
                challengerDragon.fighting.wins = (challengerDragon.fighting.wins || 0) + 1;
                opponentDragon.fighting.losses = (opponentDragon.fighting.losses || 0) + 1;
            } else {
                // Opponent won
                opponentDragon.fighting.wins = (opponentDragon.fighting.wins || 0) + 1;
                challengerDragon.fighting.losses = (challengerDragon.fighting.losses || 0) + 1;
            }
        }

        // Save both dragons
        await Promise.all([
            challengerDragon.save(),
            opponentDragon.save()
        ]);
    } catch (error) {
...
export const getDragonFights = async (req, res) => {
    try {
        ...
        const fights = await Fight.find({
            $or: [
                { 'challenger.dragon': dragonId },
                { 'opponent.dragon': dragonId }
            ]
        })
...
export const getDragonFightStats = async (req, res) => {
    try {
        ...
        const totalFights = (fighting.wins || 0) + (fighting.losses || 0) + (fighting.draws || 0);
        const winRate = totalFights > 0 ? ((fighting.wins / totalFights) * 100).toFixed(2) : 0;
...
export const cancelFight = async (req, res) => {
    try {
        ...
        if (fight.status !== 'pending') {
            return res.status(400).json({ message: `Cannot cancel a fight that is already ${fight.status}` });
        }
        ...
        if (riderId) {
            ...
            if (!isUserInvolved) {
                return res.status(403).json({ message: 'Only riders involved in the fight can cancel it' });
            }
        }
